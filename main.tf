...
output "master-ip" {
    value = ["${google_compute_instance.kube-master.network_interface.*.access_config.*.nat_ip}"]
} 


output "minion-ips" {
    value = ["${google_compute_instance.kube-minion.network_interface.*.access_config.*.nat_i}"]
} 
